...
if __name__ == '__main__':
    ...
    parser.add_argument('--ratings_data_path',
                        type=str,
    ...
    parser.add_argument('--random_seed',
                        type=int,
                        help='Seed for dataset split generation',
                        default=0)
    ...
    df['genres_id'] = df_explode[['movieId', 'genres_id']].groupby('movieId')['genres_id'].apply(list).values
    # re-label columns
    ...
    df_train[['user_id','movie_id','timestamp']].to_parquet(os.path.join(OUTPUT_FOLDER_PATH, "movielens_x_train.pk"))
    df_train[['rating']].to_parquet(os.path.join(OUTPUT_FOLDER_PATH, "movielens_y_train.pk"))
    df_test[['user_id','movie_id','timestamp']].to_parquet(os.path.join(OUTPUT_FOLDER_PATH, "movielens_x_test.pk"))
    df_test[['rating']].to_parquet(os.path.join(OUTPUT_FOLDER_PATH, "movielens_y_test.pk"))
